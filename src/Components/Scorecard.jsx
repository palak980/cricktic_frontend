...
function Scorecard() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    setIsLoading(true);
    axios.get('https://liveupcomingpro-production.up.railway.app/cricinfo/scorecard/')
      .then(response => {
    ...
    <div className='container-fluid py-3' id='AdminEmp'>
      ...
      {selectedScorecard && (
        <div className='container'>
          <div className="innings-buttons my-3 d-flex justify-content-center">
            <button
            style={{width:'150px',textAlign:'center'}}
              className={`btn btn-none border-secondary mx-2 mt-3  ${showFirstInnings ? 'active' : ''}`}
            ...
            <button
            style={{width:'150px',textAlign:'center'}}
              className={`btn btn-none border-secondary mx-2 mt-3  ${showSecondInnings ? 'active' : ''}`}
          ...
          {showFirstInnings && (
            ...
            <div className="1stInnings border p-3 rounded my-3">
              <h3 id='h1' className='text-center'>1st Innings</h3>
              {Object.keys(selectedScorecard[0][0]).slice(0, 1).map((key, index) => (
                  ...
                  <tbody>
                    ...
                    {selectedScorecard[0].map((batter, batterIndex) => (
                      <tr key={batterIndex}>
                        ...
                        <td className="px-3">{batter[(Object.keys(batter)[0])]}</td>
                        <td className="px-3">{batter['R']}</td>
                        <td className="px-3">{batter['B']}</td>
                        <td className="px-3">{batter['4s']}</td>
                        <td className="px-3">{batter['6s']}</td>
                        <td className="px-3">{batter['SR']}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className='table table-sm w-100'>
                  ...
                  <tbody>
                    {selectedScorecard[2].map((bowler, bowlerIndex) => (
                      <tr key={bowlerIndex}>
                        <td className="px-3">{bowler[(Object.keys(bowler)[0])]}</td>
                        <td className="px-3">{bowler['O']}</td>
                        <td className="px-3">{bowler['M']}</td>
                        <td className="px-3">{bowler['R']}</td>
                        <td className="px-3">{bowler['W']}</td>
                        <td className="px-3">{bowler['Econ']}</td>
                        <td className="px-3">{bowler['Dots']}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {showSecondInnings && (
            ...
            <div className="2ndInnings border p-3 rounded" id='AdminEmp'>
              <h3 id='h1' className='text-center'>2nd Innings</h3>
              {Object.keys(selectedScorecard[3][0]).slice(0, 1).map((key, index) => (
                <h3 id='h1' key={index} className='px-3'>{key.replace('Batters', '')}</h3>
              ))}
              <div className="table-responsive">
